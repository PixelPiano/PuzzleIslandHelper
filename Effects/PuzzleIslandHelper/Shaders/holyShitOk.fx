...
float createBump(float2 p,float2 bump, float2 area, float thresh)
{
    float r = length(area) / (bump + length(p + bump));

        return clamp(thresh - r, -thresh,thresh) / thresh * 10;
...
float opCheapBend(in float3 p ,float3 t, float3 s, float b, float bend)
{
    float co = cos(bend*p.x);
    float si = sin(bend*p.x);
...
float distanceToNearestSurface(float3 ray)
{
    ...
    return opCheapBend(ray, 0.05, size, 15,Time);
