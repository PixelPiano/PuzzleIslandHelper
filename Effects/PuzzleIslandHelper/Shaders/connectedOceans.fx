...
float4 SpritePixelShader(float2 uv : TEXCOORD0) : COLOR0
{
	...
	float len = length(o);
	float angle = atan2(uv.x, uv.y) + (Time - 10) / 20;
	float a = fmod(PI * angle / dist,dist);
	color = (smoothstep(color - 0.1, color, a) - smoothstep(color, color + 0.1, a)) * float4(0.2, 0, 0.3, 1);
	//color = lerp(color,(float4)1, a);
	//color = lerp(color,(float4)1, uv.x);
	return color;
}
