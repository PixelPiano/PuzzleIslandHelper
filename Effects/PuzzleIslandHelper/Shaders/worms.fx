...
float3 thatCoolCloudThing(float3 p, float t)
{
    float z = 0;
    float4 c = tex(p);
    z = c.r + c.g + c.b;
    return float3(0,0,clamp(0.5, 0.9, z));
}
...
float getZDisplacement(float2 uv, float3 p, float t)
{
    uv = float2(pnoise(uv+Time * 0.3,10), pnoise(p,10.));
    //uv += iTime;
    float d = uv.x - uv.y;
    d *= d;
    d *= 10.;
...
float3 eq(float2 uv, float3 p, float t)
{
    return float3(0,0,getZDisplacement(uv,p, t));
}

float3 displacement(float2 uv, float3 p, float3 n, float h)
{
    float s = Time;
    float3 e = eq(uv,p, Time);
...
float3 sdPlane( float2 uv, float3 p, float3 n, float h )
{
    float3 d1 = dot(p,n) + h; //plane equation
    //return d1;
    float3 d2 = displacement(uv, p, n, h);
    return d1 + dot(d2, n); //each displacement should only affect the plane passed in
}
//=====================================================

float distanceToNearestSurface(float2 uv, float3 ray)
{
    ...
    float pf = sdPlane(uv, ray, float3(0.0, 0.0, 1.0),-1);   //front plane
    return pf;
    //return min(min(min(pl, pr), min(pt,pb)),pf); //return the smallest distance
}
float3 computeSurfaceNormal(float2 uv, float3 hit, float s){
    float d = distanceToNearestSurface(uv,hit);

    return normalize(float3(
        distanceToNearestSurface(uv,hit+deltax * s)-d,
        distanceToNearestSurface(uv,hit+deltay * s)-d,
        distanceToNearestSurface(uv,hit+deltaz * s)-d
    ));
}
float3 intersectWithWorld(float2 uv, float3 cam, float3 dir)
{
    float dist = 0;
    ...
    for(int i = 0; i < 20; i++)
    {
        ...
        float nearest = distanceToNearestSurface(uv, hit);
        if(nearest < 0.01)
        {
            float3 normal = computeSurfaceNormal(uv, hit, 0.1);
...
{  
	...
	float3 rayDirection = normalize(pixel - cameraPosition);
  	...
  	float3 curve = intersectWithWorld(uv, cameraPosition, rayDirection);
