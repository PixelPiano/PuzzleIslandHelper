...
float4 SpritePixelShader(float2 uv : TEXCOORD0) : COLOR0
{
    ...
    float4 red = SAMPLE_TEXTURE(text, uv + float2(RedOffset,0));
    float4 green = SAMPLE_TEXTURE(text, uv + float2(GreenOffset,0));
    float4 blue = SAMPLE_TEXTURE(text, uv + float2(BlueOffset,0));
