...
uniform float EyeTimer;
uniform float2 RandomPosition;
...
float4 SpritePixelShader(float2 uv : TEXCOORD0) : COLOR0
{
    ...
    float4 color = SAMPLE_TEXTURE(text,neww);
    color += (float4)r * Amplitude;
    float l = min(EyeTimer,1);
    float a =  0.7 * (Dimensions.x/ Dimensions.y);
    float b = lerp(0, 0.4, l);
    float x = uv.x + (RandomPosition.x/Dimensions.x) - 0.5;
    float y = uv.y + (RandomPosition.y/Dimensions.y) - 0.5;
    
    float angle = 3.1415;
    float x2 = x * cos( angle ) + y * sin( angle );
    float y2 = y * cos( angle ) - x * sin( angle );
	
	float distance = pow( y, 2.0 ) / ( a * a ) + pow( x, 2.0 ) / ( b * b );
    if ( distance <= 0.1)
    {
        if(distance<=0.07)return float4( 0.0, 0.0, 0.0, 1.0 );
