...
{
    return float4((float3)((color.r + color.g + color.b)/3.0f), color.a);
}
...
float4 SpritePixelShader(float2 uv : TEXCOORD0) : COLOR0
{
    ...
    float amplitude = sin(Time) + Time;
    float4 color =  Greyscale(SAMPLE_TEXTURE(text, uv));
    
    color.rgb =  hash3(uv + amplitude);
    amplitude = Time % 1;
    float space = 0.1;
    float amount = space * amplitude;
    float stroke = 0.005;
    float modded = (uv.y % space);
    if(modded < amount + stroke && modded > amount - stroke)
    {
        return float4(color.rgb * 0.8, 1);
    }
    return color;
}
