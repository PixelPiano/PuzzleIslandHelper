...
float Circle(float2 uv, float2 center, float2 size, float dir, bool shudder)
{
	...
	{
		float ra = rand2(Time);
		float ra2 = rand3(Time);
		r += (0.009 * float2(ra,ra2));
	}
...
float4 SpritePixelShader(float2 uv : TEXCOORD0) : COLOR0
{
	...
	float s = sin(Time);
	float dist = size - Circle(uv, (float2)0.5, size, s, false);
	...
	if(inHeart)
	{
		if(Time % 0.15 < 0.075)
		{
			float r = rand2(uv * floor(Time * (uv.x > 0.5 ? 3 : 6)));
			alpha *= 0.9 + r * 0.3;
		}
	}
	return color * alpha;
