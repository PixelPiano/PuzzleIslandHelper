...
float4 SpritePixelShader(float2 uv : TEXCOORD0) : COLOR0
{
    ...
    float strength  = 0; // Strength of the CRT TV effect
    ...
    chrom.x = sin(Time)*2.0% 0.0025;
    chrom.y = sin(Time)*1.5 % 0.0025;
    
    float4 col = float4(0,0,0,0);
    col.r       = SAMPLE_TEXTURE(text, float2(neww.x+chrom.x, neww.y + chrom.y)).r;
    col.g       = SAMPLE_TEXTURE(text, float2(neww.x, neww.y)).g;
    col.b       = SAMPLE_TEXTURE(text, float2(neww.x-chrom.x, neww.y - chrom.y)).b;
