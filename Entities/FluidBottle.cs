...
using System.Linq;
using static Celeste.Mod.PuzzleIslandHelper.Entities.PotionFluid;
       ...
       => DoRaycast(scene.Tracker.GetEntities<Solid>().Select(s => s.Collider), start, end);
        ...
        public override void Update()
        {
            ...
            else
            {
                ...
                if (base.Center.X > l.Bounds.Right)
                {
                    MoveH(32f * Engine.DeltaTime);
                    if (base.Left - 8f > l.Bounds.Right)
                    {
                        RemoveSelf();
                    }
                }
                else if (base.Left < l.Bounds.Left)
                {
                    base.Left = l.Bounds.Left;
                    Speed.X *= -0.4f;
                }
                else if (base.Top < l.Bounds.Top - 4)
                {
                    base.Top = l.Bounds.Top + 4;
                    Speed.Y = 0f;
                }
                else if (base.Bottom > l.Bounds.Bottom && SaveData.Instance.Assists.Invincible)
                {
