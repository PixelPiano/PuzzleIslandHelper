...
namespace Celeste.Mod.PuzzleIslandHelper.Entities
{
    public static class WARP
    {
        ...
        public class InputMachine : Entity
        {
            ...
            public static MTexture Texture => GFX.Game[DefaultPath + "control"];
            public static MTexture FilledTex => GFX.Game[DefaultPath + "controlFilled"];
            public Image Image;
            public Image Screen;
        ...
        public class WarpCutscene : CutsceneEntity
        {
            ...
            public WarpCutscene(WarpCapsule parent, Player player, string room, Vector2 newposition) : base()
            {
            ...
            public WarpCutscene(WarpCapsule parent, Player player, string room) : base()
            {
                FirstParentID = parent.ID;
                Parent = parent;
                Player = player;
            ...
            public void InstantTeleport(Level level, Player player, Action<Level, Player> onEnd = null)
            {
                ...
                level.OnEndOfFrame += delegate
                {
                    ...
                    level.Camera.Position = level.Tracker.GetEntity<WarpCapsule>() is var r ? r.Position + camPos : level.LevelOffset + camPos;
            ...
            private IEnumerator Routine(Player player)
            {
                ...
                yield return Parent.SendPlayerRoutine(player); //tell capsule to start preparing for warp
                if (Parent.UsesRune) Returning = PianoModule.SaveData.VisitedRuneSites.Contains(Data.Rune); //store if player has warped here before
                ...
                DoorPercentSave = Parent.DoorPercent; //save the percent the doors are closed
        ...
        public class Door : Entity
        {
            private class Lock : Entity
            {
                ...
                public Lock(Door door) : base(door.Position)
                {
                    Door = door;
                    Depth = 7;
                    Image = new Image(GFX.Game[DefaultPath + "lock"]);
            ...
            private string path;
            public Door(Vector2 position, int xScale, float xOffset, string path) : base(position)
            {
                this.path = path;
                Depth = 8;
                Orig = position;
        ...
        public class UI : Entity
        {
            ...
            public override void Update()
            {
                ...
                if (!Standby)
                {
                    if (global::Celeste.Input.DashPressed)
                    {
                        FadeOut(true);
                    }
            ...
            public void CheckRune()
            {
                ...
                if (RuneExists(rune, out WarpCapsuleData data))
                {
        ...
        public static bool RuneExists(Rune input) => RuneExists(input, out _);

        public static WarpCapsuleData GetWarpData(Rune rune) => rune == null || Engine.Scene is not Level level ? null : Data[level.GetAreaKey()].GetDataFromRune(rune);
        ...
        public abstract class WarpCapsule : Entity
        {
            ...
            public float DoorPercent, DoorStallTimer, ShineAmount;
            public bool InCutscene, Enabled = true, DoorsIdle = true, InvertFlag, Accessible, Blocked;
            public string RoomName, DisableFlag;
            public EntityID ID;
            public Image Bg, Fg;
            public DotX3 Talk;
            ...
            public WarpCapsule(EntityData data, Vector2 offset, EntityID id) : this(data.Position + offset, id, data.Attr("disableMachineFlag"), data.Bool("invertMachineFlag"), data.Attr("path"))
            {
            }
            public WarpCapsule(Vector2 position, EntityID id, string disableFlag, bool invertFlag, string path = null, bool usesRune = false, bool usesBeam = false) : base(position)
            {
            ...
            public override void Awake(Scene scene)
            {
                ...
                LeftDoor = new Door(Center, -1, XOffset, Path);
                RightDoor = new Door(Center, 1, XOffset, Path);
            ...
            public IEnumerator SendPlayerRoutine(Player player)
            {
                player.StateMachine.State = Player.StDummy;
                InstantOpenDoors();
        ...
        [Tracked]
        public class WarpCapsule2 : WarpCapsule
        {
            ...
            public WarpCapsule2(EntityData data, Vector2 offset, EntityID id)
                : base(data.Position + offset, id, data.Attr("disableMachineFlag"), data.Bool("invertMachineFlag"), null, true, true)
            {
            ...
            public override void Interact(Player player)
            {
                ...
                if (data != null)
                {
                    Scene.Add(new WarpCutscene(this, player, data.Room, data.Position));
                }
