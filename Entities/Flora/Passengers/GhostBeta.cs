...
{
    [CustomEntity("PuzzleIslandHelper/Passengers/GhostBeta")]
    [Tracked]
    public class GhostBeta : VertexPassenger
    {
        ...
        public GhostBeta(EntityData data, Vector2 offset, EntityID id) : this(data, offset, id, 16, 16, Vector2.One, new(-1, 1), 0.95f)
        {
        }
        public GhostBeta(EntityData data, Vector2 offset, EntityID id, float width, float height, Vector2 scale, Vector2 breathDirection, float breathDuration) :
            base(data, offset, id, width, height, scale, breathDirection, breathDuration)
        {
            MinWiggleTime = 1;
            MaxWiggleTime = 2.5f;
