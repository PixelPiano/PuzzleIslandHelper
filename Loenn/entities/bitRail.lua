...
bitRail.name = "PuzzleIslandHelper/Bitrail"
bitRail.nodeLimits = {1, -1}
bitRail.minimumSize = {8, 8}
bitRail.nodeVisibility = "never"
bitRail.nodeLineRenderType = "line"
bitRail.depth = -11000
...
local function addStraightVerticalSprites(sprites, texture, x, y, width, length, direction, startNode, endNode)
    for oy = 0, length - 1, 8 do
        local tex = straightTextureQuads["vertical"]
...
local function addCornerSprites(sprites, texture, x, y, width, aaa, length, direction, cornerType, startNode, endNode)
    ...
    local nonWallQuad = straightTextureQuads["horizontal"]
    ...
    for ox = 0, 8 - 1, 8 do
        for oy = 0, 8 - 1, 8 do
            local quad = cornerQuad
            addSprite(sprites, texture, x + ox + offsetX, y + oy + offsetY, quad[1], quad[2])
        end
    end
end

local function addRailSectionSprites(sprites, entity, texture, px, py, nx, ny, nnx, nny, width, startNode, endNode)
...
local function addRailSprites(sprites, entity)
    local texture = "objects/PuzzleIslandHelper/bitRail/rails"
    ...
    for i, node in ipairs(nodes) do
        ...
        addRailSectionSprites(sprites, entity, texture, px, py, nx, ny, nnx, nny, width, startNode, endNode)
...
function bitRail.sprite(room, entity)
    return addRailSprites({}, entity)
end

function bitRail.selection(room, entity)
    local nodes = entity.nodes or {}
