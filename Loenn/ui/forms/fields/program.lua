...
program.fieldType = "PuzzleIslandHelper.program"
...
function program._MT.__index:getCurrentText()
    return self.currentText or self.field.text or ""
end

function program._MT.__index:fieldValid()
   local v= self:fieldsValid()
   return v[1] and v[2] and v[3]
end
function program._MT.__index:validateIdx(v)
    return true
end
local function validateType(v)
    local lower = string.lower(v)
    for i = 1, #validProgramTypes do
        if lower == validProgramTypes[i] then
            return true
        end
    end
    return false
end
function program._MT.__index:fieldsValid()
...
function program._MT.__index:setValue(value)
    self.currentTexts = {
...
function program._MT.__index:getValue()
...
local function getLabel(value)
    ...
    if value[2] ~= nil then
        text = text .. ", Tab: "..value[2]
    end
    if value[3] ~= nil then
        text = text .. ", Window: "..value[3]
    end
    return text

end
function program.getElement(name, value, options)
    local formField = {}
    ...
    local tab = uiElements.field(tostring(value[2]), fieldChanged(formField,2)):with({
        minWidth = nMinWidth,
        maxWidth = nMaxWidth
    })
    local window = uiElements.field(tostring(value[3]),fieldChanged(formField,3)):with({
        minWidth = nMinWidth,
        maxWidth = nMaxWidth
    })
    ...
    formField = setmetatable(formField, program._MT)
